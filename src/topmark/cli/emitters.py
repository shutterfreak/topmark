...
#   project      : TopMark
#   file         : emitters.py
#   file_relpath : src/topmark/cli/emitters.py
#   license      : MIT
...
def emit_banner(
    *,
    ...
    """Emit the initial banner for a command.
...
def render_file_summary_line(
    *,
    ...
    Args:
        ctx: Processing context containing status and
            configuration.
...
def emit_summary_counts(
    *,
    ...
    """Emit the human summary (aligned counts by outcome), colored."""
    console: ConsoleLike = get_console_safely()
    console.print()
...
def emit_per_file_guidance(
    *,
    ...
    """Emit one human guidance line per result (when not in --summary)."""
    console: ConsoleLike = get_console_safely()
...
def render_diff(
    *,
    ...
    """Render a unified diff for a changed file in human output mode, in color.
...
def emit_diffs(
    *,
    ...
    """Print unified diffs for changed files in human output mode, in color.
