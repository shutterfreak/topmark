...
"""Machine-output payload builders for TOML provenance exports.

This module contains pure helpers that build strongly typed payload objects
(dataclasses) for machine-readable TOML provenance output.
...
This module performs no I/O and does not shape envelopes or records.
"""
...
def build_toml_provenance_payload(
    ...
    Raises:
        ValueError: If config provenance layers do not align with resolved TOML sources.
    ...
    for layer in layers:
        ...
        if layer.kind == ConfigLayerKind.DEFAULT:
            toml_fragment = _normalize_toml_fragment(build_default_topmark_toml_table())
            ...
            )  # Raises TomlRenderError: If an invalid files serialization mode is specified.
        else:
