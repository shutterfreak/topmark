...
@dataclass(frozen=True, kw_only=True, slots=True)
class ProcessingDetailSnapshot:
    """Durable report-detail facts captured from a processing context.

    This snapshot intentionally stores only reduced, reportable values needed
    after pipeline execution. It must not retain volatile view objects, original
    or updated file images, processors, configuration objects, structured diff
    planning metadata, or other runtime state owned by
    [`ProcessingContext`][topmark.pipeline.context.model.ProcessingContext].

    Unified diff text is intentionally retained as plain `str | None` because
    current public API, CLI presentation, and machine-output contracts expose
    unified-diff text rather than structured diff operations. A non-`None`
    value is detached durable result detail and remains valid after
    [`DiffView`][topmark.pipeline.views.DiffView] has been released.

    Attributes:
        diff_text: Unified diff text generated by the patcher, if one is
            available. `None` means no durable diff detail was generated.
    """
    ...
    def from_context(
        ...
        """Create a durable detail snapshot from a processing context.

        Args:
            ctx: Source mutable context.

        Returns:
            Reduced detail snapshot detached from context-owned views.
        """
        diff_text: str | None = None
        if ctx.views.diff is not None:
            diff_text = ctx.views.diff.text
        return cls(diff_text=diff_text)

    def to_dict(self) -> dict[str, object]:
        """Return a machine-readable detail payload.

        Returns:
            JSON-serializable mapping containing reduced report-detail facts.
        """
        return {
