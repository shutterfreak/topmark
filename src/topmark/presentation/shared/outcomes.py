...
def collect_outcome_counts_styled(
    ...
    This result-compatible helper accepts durable processing results. Each result
    carries its own execution-mode snapshot, so mixed result batches are classified
    per result without assuming a run-global apply mode.

    Args:
        results: Durable results to classify and count.
