...
| Behavioral config | `max_header_line_length`                           | Replace if explicitly set                              | Replace                              | Optional positive soft width; absent child layers inherit the nearest configured value.                                                          |
| Behavioral config | `wrap_fields`                                      | Replace when present, including empty                  | Replace                              | Ordered wrapping allowlist; an explicit empty child list clears inherited selection.                                                             |
...
| `mixed_line_endings`                 | Reject mixed physical terminators by default or preserve every existing non-header terminator (`preserve`)                                                                           |
