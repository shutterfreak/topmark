...
Pipelines operate on a resolved configuration
(\[`FrozenConfig`\][topmark.config.model.FrozenConfig]) and runtime options assembled via the TOML →
...
execution. They influence configuration discovery and validation behaviour, but do not become
