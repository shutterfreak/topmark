...
in an actionable, noncompliant, or all-results report view. Human and machine detail rendering still
consume contexts while TopMark lacks a durable detail snapshot for reportable facts such as diff
text. That is transitional rather than a target architecture. A later result-boundary step can add
optional detail snapshots, including `diff_text` generated by
\[`ProcessingResult.from_context()`\][topmark.pipeline.result.ProcessingResult.from_context], so
volatile views can be released immediately after result reduction.
