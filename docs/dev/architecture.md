...
`detail.diff_text` field. TEXT, Markdown, JSON, and NDJSON check/strip detail output therefore no
longer project from live \[`ProcessingContext`\][topmark.pipeline.context.model.ProcessingContext]
instances once the reduction boundary has been crossed. Probe rendering and probe machine output
remain context-based because probe details still live on resolver-specific context state. Unified
diff headers generated by the patcher also remain pre-reduction context consumers because they are
created while mutable views still own the planned patch content.
...
\[`RunResult`\][topmark.api.types.RunResult] DTOs. The check/strip CLI machine-output path now uses
those same durable snapshots for per-file JSON and NDJSON processing results. Probe output still
depends on resolver-specific probe state stored on contexts.
