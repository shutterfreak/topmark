...
Runtime configuration, registry overlays, runtime resolution state, and file type selection do not
affect the reported version output.
...
- TEXT-oriented human-readable output supports verbosity (`-v`).
...
- `-v`, `--verbose` increases TEXT-oriented human-readable output detail.
- Markdown output ignores TEXT verbosity and always renders a complete document.
- Machine-readable JSON and NDJSON output are unaffected by TEXT verbosity.
...
As with TEXT-oriented human-readable output, the reported version is resolved at runtime from
installed package metadata / the generated version module rather than from a manually maintained
static field in `pyproject.toml`.
...
Produces a single machine-readable JSON document:
...
- **Unexpected machine-readable formatting**: use `--output-format json` or `--output-format ndjson`
