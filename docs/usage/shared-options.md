...
When `--diff` is requested, presentation depends on the selected output format. TEXT and Markdown
render unified diffs for human review. JSON detail output embeds structured per-result `diff`
...
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`check`](commands/check.md)                                                                | File input, filtering, configuration loading, strictness, reporting, diff preview, `--apply` (mutually exclusive with `--diff`), write mode, header generation/update policy, generated-header formatting | N/A beyond normal validation conflicts                                                                                                                                                            |
| [`strip`](commands/strip.md)                                                                | File input, filtering, configuration loading, strictness, reporting, diff preview, `--apply` (mutually exclusive with `--diff`), write mode, shared content-probe runtime policy                          | Header generation/update policy and generated-header formatting                                                                                                                                   |
| [`probe`](commands/probe.md)                                                                | File input, filtering, configuration loading, strictness, output format, shared content-probe runtime policy                                                                                              | `--apply`, write mode, diff preview, human-readable summary/reporting, header generation/update policy, generated-header formatting, stdin option flags (use '-' plus `--stdin-filename` instead) |
...
filesystem location. In path-processing mode, `--apply` and `--diff` are mutually exclusive because
`--diff` is a preview mode while `--apply` performs mutation.
