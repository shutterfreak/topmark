...
- `results`: one entry per processed file (see **Per-file result payload** below). When `--diff` is
  requested, each changed result may include an optional structured `diff` object.
...
- Ordering is deterministic: outcomes follow the internal
  ...
  each outcome.
- Summary mode suppresses per-file detail. When `--diff` is requested together with machine-readable
  summary output, TopMark emits a warning on `stderr` and omits per-file diff payloads from the JSON
  document.
...
{"kind": "diff",
 "meta": { /* MetaPayload */ },
 "diff": { "path": "README.md", "diff_text": "--- README.md ..." } }
```

The `diff` record is emitted only in detail mode when `--diff` is requested and a retained unified
...
- The stream begins with:
  ...
  1. zero or more `diagnostic` records (each with `domain="config"`, using the shared
     ...
     \[`topmark.core.machine.schemas.MachineDomain`\][topmark.core.machine.schemas.MachineDomain])
- Then either:
  - detail mode: one `result` record per file, optionally followed immediately by one standalone
    `diff` record when `--diff` is requested and a diff is available for that file
  - summary mode: one `summary` record per `(outcome, reason)` bucket; per-file `result` and `diff`
    records are omitted
...
- optional diagnostics and diff detail (per-file):
  - list of diagnostics (when requested / enabled)
  - pre-computed diagnostic counts
  - `diff`, a JSON detail-mode object emitted only when `--diff` is requested and a retained unified
    diff is available for that result

When JSON detail mode is combined with `--diff`, changed results may include an embedded `diff`
object:

```jsonc
{
  "path": "README.md",
  "status": { /* per-axis status objects */ },
...
The embedded JSON `diff` object does not repeat `path` because the parent result already identifies
the file. In NDJSON detail mode, the `result` record does not embed `diff`; instead, TopMark emits
an adjacent standalone `diff` record that includes both `path` and `diff_text`.

> [!NOTE]
>
> - The `--diff` option is supported for machine-readable output. JSON detail output embeds
>   structured diff payloads under affected per-file results, while NDJSON detail output emits
>   adjacent standalone `diff` records.
> - Machine summary mode intentionally omits per-file diff payloads even when `--diff` is requested.
>   TopMark emits a warning on `stderr` to make that suppression explicit.
> - ANSI coloring is human-only and is never emitted by machine-readable formats.
> - Unified diff rendering remains a human presentation feature for TEXT and Markdown output.
>   Machine-readable consumers should use JSON/NDJSON fields rather than relying on TEXT, Markdown,
>   or terminal-rendered unified diff blocks.
