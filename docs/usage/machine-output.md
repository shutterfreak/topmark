...
- optional diagnostics and reduced detail (per-file):
  - list of diagnostics (when requested / enabled)
  - pre-computed diagnostic counts
  - `detail.diff_text`, a reduced detail field that remains `null` unless a durable diff detail was
    generated by an internal processing path

> [!NOTE]
>
> - The `--diff` option and any ANSI coloring are **human-only** and are rejected for
>   machine-readable output formats. Machine-readable result payloads expose reduced detail state
>   under `detail`, but normal CLI JSON/NDJSON processing output does not render unified diff
>   blocks.
