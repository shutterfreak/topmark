...
These tests exercise discovery, report-scope filtering, conflict validation,
and write behavior (`apply=True`) for `check()` and `strip()`. Registry-
specific tests live in dedicated modules under `tests/api/` to keep concerns
separate.
"""
...
def test_apply_check_writes_when_needed(
    ...
    assert any(
        fr.outcome
        in {topmark.core.outcomes.Outcome.WOULD_INSERT, topmark.core.outcomes.Outcome.WOULD_UPDATE}
        for fr in r0.files
    )
...
def test_strip_removes_header(repo_py_with_header: Path, proc_py: HeaderProcessor) -> None:
    ...
    assert has_header(target.read_text(encoding="utf-8"), proc_py, "\n")
    r0: api.RunResult = api.strip(
        [target],
        apply=False,
        include_file_types=["python"],
    )
    # assert any(fr.outcome.value in {"would_change", "changed"} for fr in r0.files)
    assert any(fr.outcome == topmark.core.outcomes.Outcome.WOULD_STRIP for fr in r0.files)
...
def test_include_file_types_argument_filters(tmp_path: Path) -> None:
    """`include_file_types` argument narrows discovery to specific types."""
    (tmp_path / "a.py").write_text("print('a')\n", encoding="utf-8")
