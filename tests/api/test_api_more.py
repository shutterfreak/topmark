...
def test_skip_unsupported_hides_unsupported_files(repo_py_toml_xyz_no_header: Path) -> None:
    ...
    r_skip: api.RunResult = api.check(
        paths,
        apply=False,
        include_file_types=None,
...
def test_include_file_types_param_overrides_config(repo_py_toml_xyz_no_header: Path) -> None:
    """Test 'include_file_types: tuple[str, ...] = ()' override in config and API.

    When both config.files.include_file_types: tuple[str, ...] = () and
    include_file_types: tuple[str, ...] = () param are provided,
    ...
    r: api.RunResult = api.check(
        [src],
        apply=False,
        config=cfg(files={"include_file_types: tuple[str, ...] = ()": ["python", "toml"]}),
