...
def test_skip_unsupported_hides_unsupported_files(repo_py_toml_xyz_no_header: Path) -> None:
    ...
    """
    src = repo_py_toml_xyz_no_header / "src"
    paths = [src / "without_header.py", src / "note.xyz"]

    r_no_skip = api.check(paths, apply=False, file_types=None, skip_unsupported=False)
    paths_no_skip = {fr.path for fr in r_no_skip.files}
    ...
    r_skip = api.check(paths, apply=False, file_types=None, skip_unsupported=True)
    paths_skip = {fr.path for fr in r_skip.files}
...
def test_file_types_param_overrides_config(repo_py_toml_xyz_no_header: Path) -> None:
    ...
    seen = {fr.path for fr in r.files}
