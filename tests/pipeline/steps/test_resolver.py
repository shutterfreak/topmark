...
def test_resolve_python_file_resolves_with_processor(
    ...
    assert ctx.status.file is FileStatus.RESOLVED
...
def test_resolve_unknown_extension_marked_unsupported(
    ...
    assert ctx.status.file is FileStatus.SKIPPED_UNSUPPORTED
...
def test_resolve_sets_skip_when_no_processor_registered(
    ...
    assert ctx.file_type is not None
    assert ctx.header_processor is None
    assert ctx.status.file is FileStatus.SKIPPED_NO_HEADER_PROCESSOR
...
def test_resolve_respects_skip_processing_filetype(
    ...
    assert ctx.header_processor is None
    assert ctx.status.file is FileStatus.SKIPPED_KNOWN_NO_HEADERS
...
def test_resolve_pattern_fullmatch_not_search(
    ...
    ctx1 = resolve(ctx1)
    assert ctx1.status.file is FileStatus.RESOLVED
    ...
    ctx2 = resolve(ctx2)
    assert ctx2.status.file is FileStatus.RESOLVED
