...
def _write_internals_summary(summary_path: str, groups: dict[str, list[str]]) -> None:
    ...
    The internals sidebar intentionally lists only the internals landing page,
    top-level packages, and top-level modules. Nested modules remain
    discoverable through generated package indexes, breadcrumbs, direct links,
    and search. This avoids a very dense, mostly flat sidebar for generated
    internals documentation.

    Args:
        summary_path: Docs-relative path for the generated SUMMARY.md file.
        groups: Mapping of top-level `topmark.*` segment to discovered modules.
    """
    top_level_packages: list[tuple[str, str]] = []
    top_level_modules: list[tuple[str, str]] = []
    ...
    for group in sorted(groups):
        ...
        if _is_package(pkg):
            ...
            top_level_packages.append((pkg, rel_entry))
        else:
    ...
    with mkdocs_gen_files.open(summary_path, "w") as fd:
        ...
        if top_level_packages:
            ...
            for pkg, rel_entry in top_level_packages:
                fd.write(f"    - [{pkg}]({rel_entry})\n")

        if top_level_modules:
            fd.write("- Top-level modules\n")
            for mod, rel_entry in top_level_modules:
                fd.write(f"    - [{mod}]({rel_entry})\n")
